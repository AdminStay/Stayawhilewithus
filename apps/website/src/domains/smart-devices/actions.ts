...
export async function sendAugustLockCommandAction(
  ...
  // Revalidate on every real, on-record outcome (2026-09-24, extended
  // 2026-09-25 for "ambiguous") — not just success. A FAILED or AMBIGUOUS
  // first-verification test must immediately re-derive this device as
  // BLOCKED (computeLockControlEligibility/computeFirstTestEligibility
  // both read AuditLog history fresh on every render), so the next page
  // render can never still offer the same not-yet-refreshed "Test
  // controllability" control for a device whose outcome just became
  // uncertain. "already_running" is deliberately excluded — nothing was
 ...
 * in this file.
 *
 * Placed deliberately before refreshAugustAction below, not after — an
 * existing source-level test isolates that function's body by slicing from
 * its declaration to the next top-level `export`, so anything appended
 * after it (including this block's own doc comment) would otherwise be
 * misread as part of refreshAugustAction's own body.
 */
...
export async function refreshAugustTelemetrySpotAction(
  ...
  try {
    ...
    if (outcome.result === "success") {
      ...
      // without a manual reload — same pattern refreshAugustAction below
