...
import type { NestCredentials } from "./types";

/**
 * Nest integration client. Structural stub for Phase 1 — every method
  ...
  implements BaseIntegrationClient, SyncCapable, WebhookReceivable
{
  ...
  async connect(): Promise<{ connected: boolean; connectedAt: Date }> {
    throw new NotImplementedError("Nest", "connect");
  }

  async disconnect(): Promise<void> {
    throw new NotImplementedError("Nest", "disconnect");
  }

  async authenticate(): Promise<void> {
    throw new NotImplementedError("Nest", "authenticate");
  }
