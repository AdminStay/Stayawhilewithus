...
Notion is a **read-only source of truth** for Kenny & Jenny's existing information. This integration must **never** update, overwrite, delete, archive, rename, or otherwise modify any existing Notion page, database, property, or content — existing Notion data must remain exactly as it is. StayWhile may read Notion data for discovery/reference (as `sync`/`validateCredentials` already do), but any conflict or mismatch found between Notion and StayWhile's own records must be **reported to a human, never auto-resolved by writing back to Notion**. Do not implement `sync("OUTBOUND")`, any Notion `PATCH`/create/archive call, or any block-append — all such functionality stays explicitly out of scope until the client says otherwise. This is a hard constraint on this specific client's configured Notion integration only; it says nothing about how any other client's Notion integration in a different workspace should behave.
